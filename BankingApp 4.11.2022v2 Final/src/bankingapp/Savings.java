...
public class Savings extends javax.swing.JFrame {
    ...
    float balance;
    int limit;
    SavingsAccount acc2 = new SavingsAccount(name, balance, limit);
    
    /**
     * Creates new form BankGUI
     */
    public Savings(String name, float balance, int limit) {
        initComponents();
        acc2.name = name;
        acc2.balance = balance;
        acc2.limit = 0;
        UsernameText.setText(acc2.name);
    ...
    private void WithdrawConfirmActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_WithdrawConfirmActionPerformed
        // TODO add your handling code here:
        if (acc2.withdrawl(Float.parseFloat(ConfirmText.getText())) == false && acc2.limit <= 6){
                   jTextField1.setText(String.valueOf(acc2.balance));
                   acc2.limit += 1;
        }
        else {
